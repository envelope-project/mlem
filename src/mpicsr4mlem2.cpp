    ...
    const size_t updateSize)
{
    ...
    #pragma omp parallel
    {
        ...
        for(i = 1, startColumnIndex = 0; 
            ...
            for(j = startColumnIndex; j < endColumnIndex; ++j) 
                private_update[mpi.threadsCSR[tidx]->columns[j]] += 
                    mpi.threadsCSR[tidx]->values[j] * correlation[row];
        }
...
void splitMatrix(Csr4Matrix* matrix, MpiData& mpi) 
{
    ...
    #pragma omp parallel 
    {
        ...
        for (i = 0; i < mpi.threadsCSR[tidx]->nColumn; ++i) {
            mpi.threadsCSR[tidx]->columns[i] = columnsTemp->at(i);
            mpi.threadsCSR[tidx]->values[i] = valuesTemp->at(i);
        }
    ...
    free(rowidx);
        ...
        printf("\nThreads Partitioning\n"
            "Rows: %d, Columns: %d Matrix elements: %d, \n"
        ...
        printf("Total elements: %d, total Rows: %d \n", totalE, totalR);
    #endif
}
#else 
void splitMatrix(Csr4Matrix* matrix, MpiData& mpi) 
